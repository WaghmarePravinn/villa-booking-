...
const VillaCard: React.FC<VillaCardProps> = ({ villa, whatsappNumber, onViewDetails }) => {
  ...
  return (
    <div 
      ...
      className="group bg-white rounded-[3rem] overflow-hidden shadow-sm hover:shadow-[0_40px_100px_rgba(2,132,199,0.1)] transition-all duration-700 border border-sky-50 cursor-pointer flex flex-col h-full hover:-translate-y-4"
    >
      <div className="relative h-80 overflow-hidden">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-sky-900/20 via-transparent to-transparent group-hover:opacity-80 transition-opacity duration-700"></div>
        
        <div className="absolute top-8 right-8 bg-white/95 backdrop-blur-xl px-4 py-2 rounded-2xl text-sm font-black text-sky-900 shadow-2xl flex items-center gap-2 group-hover:bg-sky-600 group-hover:text-white transition-all duration-500">
        ...
        <div className="absolute top-8 left-8 flex flex-col gap-2">
          {villa.isFeatured && (
            <div className="bg-orange-500 px-5 py-2 rounded-2xl text-[10px] font-black text-white shadow-xl uppercase tracking-[0.2em] border border-orange-400">
              FEATURED SANCTUARY
            </div>
          )}
      ...
      <div className="p-10 flex flex-col flex-grow bg-white">
        ...
        <h3 className="text-3xl font-bold font-serif text-sky-900 leading-tight mb-4 group-hover:text-sky-600 transition-colors duration-500">
          {villa.name}
        </h3>
        ...
        <div className="mt-auto flex items-center justify-between">
          ...
          <button 
            ...
            className="w-14 h-14 bg-green-500 hover:bg-sky-600 text-white rounded-3xl flex items-center justify-center transition-all duration-500 shadow-xl active:scale-90"
          >
