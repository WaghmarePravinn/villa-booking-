...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ villas, settings, onAddVilla, onUpdateVilla, onDeleteVilla }) => {
  ...
  const handleManualImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setProgress({ active: true, message: 'Assets Buffered', percentage: 100, subMessage: 'Images are ready to be saved.' });
    ...
    <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-12 animate-fade overflow-hidden h-full">
      ...
      {activeTab === 'inventory' && (
        <div className="flex flex-col lg:flex-row gap-12 h-auto lg:h-[calc(100vh-280px)] overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="space-y-8 pb-20">
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center px-1">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Descriptions</label>
                  ...
                  <input type="text" placeholder="Short Summary" className="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-100 text-sm" value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} />
                  <textarea placeholder="Long Narrative" className="w-full px-5 py-4 rounded-2xl bg-slate-50 border border-slate-100 text-sm resize-none" rows={4} value={formData.longDescription} onChange={(e) => setFormData({ ...formData, longDescription: e.target.value })} />
                </div>
                  ...
                  <div className="flex overflow-x-auto gap-2 pb-2 scrollbar-hide">
                    {formData.imageUrls?.map((url, i) => (
                      <div key={i} className="relative w-20 h-20 shrink-0 group">
                        ...
                        <button type="button" onClick={() => setFormData({ ...formData, imageUrls: formData.imageUrls?.filter((_, idx) => idx !== i) })} className="absolute top-0 right-0 bg-red-500 text-white w-5 h-5 rounded-full text-[10px] opacity-0 group-hover:opacity-100 transition-opacity"><i className="fa-solid fa-xmark"></i></button>
                      </div>
                    ))}
                    {(!formData.imageUrls || formData.imageUrls.length === 0) && (
                      <div className="w-20 h-20 rounded-xl bg-slate-50 border border-dashed border-slate-200 flex items-center justify-center text-slate-300">
          ...
          <div className="flex-grow h-full overflow-y-auto no-scrollbar pb-20">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredVillas.map(v => (
                <div key={v.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 flex gap-6 group hover:shadow-xl transition-all relative overflow-hidden">
                  {v.isFeatured && <div className="absolute top-0 right-0 bg-orange-500 text-white text-[8px] font-black uppercase px-4 py-1 rounded-bl-xl shadow-sm">Featured</div>}
                  <img src={v.imageUrls?.[0]} className="w-24 h-24 rounded-2xl object-cover shrink-0 shadow-sm" />
                  <div className="flex-grow min-w-0">
                    ...
                    <div className="flex gap-2 mt-4">
                      <button onClick={() => handleEdit(v)} className="flex-1 py-2 bg-slate-50 text-slate-900 rounded-xl text-[8px] font-black uppercase hover:bg-slate-100 transition-colors">Modify</button>
